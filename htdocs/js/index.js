...
function adjust_footer_position()
{
	...
	{
		var offset = jQuery('body').height() - jQuery('#site-footer').offset().top;

		if (offset > 100)
		{
			jQuery('#site-footer').css({position: 'absolute', bottom: '0px'});
			jQuery('.footer-nav-widgets-wrapper ').css({position: 'absolute', bottom: '50px'});
		}
		else
		{
			jQuery('#site-footer').css({position: 'relative', bottom: '0px'});
			jQuery('.footer-nav-widgets-wrapper ').css({position: 'relative', bottom: '0px'});
		}
	}
	
	jQuery('.comments-header .comment-reply-title').replaceWith('');
}

function toggleTopButton() 
{
	var topButton = document.getElementById("guide-nav-top");
	
	if (topButton)
	{
		var bottom = jQuery('body').height() - 1500;
		
		topButton.style.display = ((document.body.scrollTop > 500 || document.documentElement.scrollTop > 500)
			&& (document.body.scrollTop < bottom && document.documentElement.scrollTop < bottom))
				? "block" : "none";	
	}
}
...
function goToTop()
{
	twentytwentyScrollTo(jQuery('#site-header').offset().top, null, 800);
}

var lastGuideSelected = false;

function guideNav()
{
	if (jQuery("#guide-nav").length || (jQuery(".top-button").length && jQuery('body').height() > 500))  
	{
		jQuery('<button onclick="goToTop()" id="guide-nav-top" class="button" title="Go to Top">Top</button>').appendTo('#site-content');

		window.onscroll = function() { toggleTopButton() };
		
		if (jQuery("#guide-nav").length) 
		{
			console.log("#guide-nav FOUND");
			
			jQuery('#guide-nav').on('change', function() 
			{
				console.log(this.value);
				
				var goTo = jQuery(this.value);
				
				if (goTo)
				{
					if (lastGuideSelected) lastGuideSelected.css({'background': 'none'});

					lastGuideSelected = jQuery("#guide-nav").find(":selected");
					
					if (lastGuideSelected) lastGuideSelected.css({'background': '#b1bdbd'});

					jQuery("#guide-nav").val(jQuery("#guide-nav option:first").val());

					twentytwentyScrollTo((goTo.offset().top - 40), null, 800);
				}
...
{	
	var _links = jQuery(".simcal-event-address");
	
	jQuery(".simcal-nav-button").click(function()
	{
		if (reloadTimer) clearTimeout(reloadTimer);
		
		reloadTimer = setTimeout(function() { calendarLinks(); }, 1200);			
	});
	...
	if (_links.length)
	{
		...
		{
			var _link = jQuery(this).text();
			
			if (_link.includes("http"))
			{
				var _html = jQuery(this).html();
				
				jQuery(this).html(_html.substr(_html.length, -Math.abs(_link.length)) + '<br /><a href=' + _link + ' target="_blank"> Discord Link </a><br /><br />');
			}			
		});		
	}
	
	_links = jQuery(".simcal-tooltip-content .simcal-event-title");
	
	if (_links.length)
	{
		_links.each(function() 
		{
			jQuery(this).parent().append("<br /><br />");
		});
	}
}
...
function fightNight()
{
	if (jQuery("#fight-box").length) 
	{
		jQuery("#fight-box").hover(function()
		{			
			jQuery("#fight-pass").show("slow", "linear");
		});
	}
}

function twitchViewers()
{
	var viewers = jQuery(".twitch-viewers");
	...
	if (viewers.length)
	{
		setTimeout(function()
		{
			...
			{
				jQuery(this).text(jQuery(this).text() + " viewers.");
			});
		}, 2000);		
	}
}
...
//jQuery( window ).on( "resize", adjust_footer_position );
